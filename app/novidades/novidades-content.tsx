...
export function getNovidadesMetadata(locale: Locale): Metadata {
  const canonical = localizePath("/novidades", locale);
  const title = "Vektrum | Novidades";
  const description = "Edições anteriores da newsletter Semana em IA, publicadas no site.";

  return {
    title,
    description,
    alternates: { canonical, languages: buildLanguageAlternates("/novidades") },
    openGraph: {
      title,
      description,
...
export async function NovidadesPageContent({ locale }: { locale: Locale }) {
            ...
            {issues.length === 0 ? (
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                {issues.map((issue) => (
                  <Link
                    key={issue.id}
                    href={localizePath(`/novidades/${issue.id}`, locale)}
