...
export default async function PrivacyPage({ searchParams }: PrivacyPageProps) {
  ...
  const value = Array.isArray(params.lang) ? params.lang[0] : params.lang;
  const en = value === "en";
  const diagnosisHref = en ? "/diagnostico?lang=en" : "/diagnostico";
  ...
  return (
    <div className="min-h-screen bg-background">
      <PrivacyLanguage language={en ? "en" : "pt-PT"} />
      <header className="mx-auto flex w-[calc(100%-32px)] max-w-4xl items-center justify-between py-6 sm:w-[calc(100%-64px)] sm:py-8">
        <Link href={en ? "/?lang=en" : "/?lang=pt-PT"} aria-label="Vektrum"><Image src="/vektrum-logo-transparent.png" alt="Vektrum" width={168} height={48} priority className="h-9 w-auto" /></Link>
        <Link href={en ? "/privacidade" : "/privacidade?lang=en"} className="rounded-full border border-border bg-surface px-3.5 py-2 text-xs font-bold tracking-wider text-muted hover:text-accent">{en ? "PT" : "EN"}</Link>
      </header>
