...
export function getPrivacyMetadata(locale: Locale): Metadata {
  return locale === "en"
    ? {
        ...
        description: "How Vektrum processes information submitted through the automation diagnosis and the meeting-booking form.",
        alternates: { canonical: "/en/privacidade", languages: buildLanguageAlternates("/privacidade") }
      }
    : {
        title: "Política de Privacidade | Vektrum",
        description: "Como a Vektrum trata os dados enviados através do diagnóstico de automação e do formulário de marcação de reuniões.",
...
export function PrivacyPageContent({ locale }: { locale: Locale }) {
  ...
  const sections = en ? [
    ["1. Who is responsible", `Vektrum is responsible for processing the information submitted through the automation diagnosis and the meeting-booking form. You can contact us at ${defaultSiteConfig.brand.email}.`],
    ["2. Information we collect", "We collect the company, operational, software, objective, and contact information you voluntarily provide during the diagnosis. When you book a meeting, we also collect your name, email address, the time slot you select, and any note you choose to add."],
    ["3. Why we use it", "We use this information solely to analyze your automation opportunities, prepare the requested report, deliver information about that request, and contact you in connection with it. Booking details are used only to schedule the meeting you requested and to send you the invitation."],
    ["4. Legal basis", "Processing is based on the consent you provide before submitting the diagnosis or confirming a booking. You may withdraw consent at any time without affecting processing already carried out."],
    ...
    ["6. Retention", "Diagnosis information is retained for up to 12 months after submission, unless a longer period is required by law or an ongoing business relationship justifies continued retention. Booking details remain in the corresponding calendar event for as long as we need the record of that meeting."],
    ...
    ["1. Responsável pelo tratamento", `A Vektrum é responsável pelo tratamento da informação enviada através do diagnóstico de automação e do formulário de marcação de reuniões. Pode contactar-nos através de ${defaultSiteConfig.brand.email}.`],
    ["2. Informação recolhida", "Recolhemos os dados da empresa, operação, software, objetivos e contacto que fornece voluntariamente durante o diagnóstico. Ao marcar uma reunião, recolhemos também o seu nome, endereço de email, o horário que escolhe e qualquer nota que queira acrescentar."],
    ["3. Finalidade", "Utilizamos estes dados apenas para analisar oportunidades de automação, preparar o relatório solicitado, enviar informação sobre esse pedido e entrar em contacto no âmbito do mesmo. Os dados da marcação servem apenas para agendar a reunião que pediu e enviar-lhe o convite."],
    ["4. Fundamento legal", "O tratamento baseia-se no consentimento prestado antes do envio do diagnóstico ou da confirmação de uma marcação. Pode retirar o consentimento a qualquer momento, sem afetar o tratamento já realizado."],
    ...
    ["6. Conservação", "Os dados do diagnóstico são conservados até 12 meses após o envio, salvo quando a lei exija um prazo superior ou uma relação comercial em curso justifique a sua conservação. Os dados da marcação permanecem no respetivo evento de calendário enquanto o registo dessa reunião for necessário."],
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto w-[calc(100%-32px)] max-w-3xl pb-24 pt-12 sm:w-[calc(100%-64px)] sm:pt-20">
        ...
        <p className="mt-5 max-w-2xl text-base leading-relaxed text-muted">{en ? "This notice explains how we process information submitted through Vektrum's interactive automation diagnosis and its meeting-booking form." : "Este aviso explica como tratamos a informação enviada através do diagnóstico interativo de automação e do formulário de marcação de reuniões da Vektrum."}</p>
        <p className="mt-3 text-sm text-muted">{en ? "Last updated: 18 September 2026" : "Última atualização: 18 de setembro de 2026"}</p>
